...
extern struct mutex ocn_netlock;

static struct work_struct yield_wq;
static struct work_struct yield_net_wq;

extern bool om_inited;

static struct timer_list timer;

int request_network_access(void);
int network_domain_bind_sport(unsigned short sport);

static void obd_yield_network_queues(struct work_struct *work)
{
        int ret;

        mutex_lock(&ocn_netlock);
	...
	if (ret != 0){
        	...
        	mutex_unlock(&ocn_netlock);
	}

        ret = network_domain_bind_sport(512);
        if (ret != 0){
...
{
	if (queue_id == Q_STORAGE_CMD_IN && 
			timeout <= MAILBOX_MIN_PRACTICAL_TIMEOUT_VAL) {
		schedule_work(&yield_wq);
	}

	if (queue_id == Q_NETWORK_CMD_IN &&
			timeout <= MAILBOX_MIN_PRACTICAL_TIMEOUT_VAL) {
		schedule_work(&yield_net_wq);
	}
}
...
{
	if (om_inited) {
		register_timeout_update_callback(Q_NETWORK_CMD_IN, obd_queue_timeout_update);
...
static int __init obd_init(void)
{
	...
	if (!om_inited) {
		...
		register_timeout_update_callback(Q_NETWORK_CMD_IN,
...
//						 obd_queue_timeout_update);
	...
	INIT_WORK(&yield_net_wq, obd_yield_network_queues);
